...
describe("drums", () => {
    it("empile des parties étiquetées", () => {
        const kit = drums("909").kick("x-x-").snare("--x-");
    ...
    it("remplit l'inventaire du Score avec les parties", () => {
        const score = toScore(drums("909").kick("x-x-").snare("--x-"), { bpm: 120 });
...
describe("bass / lead", () => {
    it("produit le même pattern que track()", () => {
        const sugar = bass("acid").notes("C2 G2").rhythm("x-x-");
    ...
    it("est immuable", () => {
        ...
        expect(base.pattern.length).toEqual(frac(1, 4)); // notes "C2" + rythme défaut "x" = 1 pas
    });
